...
def get_performance_test(trial_i):
    ...
    models = [
        DeepSet(in_features=10),
        DSGCN(in_features=10)
    ]
    ...
    for network in (models):
        ...
        with torch.no_grad():
            if isinstance(model, (DeepSet)):
                ...
                np.savetxt(log_dir + '/decoding_deepset_' + '_trial_' + str(trial_i), decoding, delimiter=',')

            if isinstance(model, (DSGCN)):
                output, decoding = model(features, adj_norm)
                np.savetxt(log_dir + '/decoding_deepset_gcn' + '_trial_' + str(trial_i), decoding, delimiter=',')
...
for i in range(num_trials):
    ...
    mae.append(maerr)
    mpe.append(mperr)
    print('run : ' + str(i+1) + ', RMSE : ' + str(np.around(np.array(mserr), 2)) + ', MAE : ' + str(np.around(np.array(maerr), 2)) +
...
def get_performance_test_bayesian(trial_i):
    ...
    for network in (models):
        ...
        if isinstance(model, (DSGCN)):
            embedding = np.loadtxt(log_dir + '/decoding_deepset_gcn' + '_trial_' + str(trial_i), delimiter=",")
...
mse_concat.to_csv('rmse_deepset.csv', index=False)
mae_concat.to_csv('mae_deepset.csv', index=False)
mpe_concat.to_csv('mape_deepset.csv', index=False)
