...
def run_set_transformer(num_neib, r, alg_name='vanilla'):
    num_trials = 100
    lr_ = 1e-3
    weight_decay_ = 1e-4
    EPOCHS = 200
    ...
    def get_performance_test(trial_i):
        ...
        if alg_name == 'vanilla':
            models = [
                SetTransformer(in_features=94)
            ]
        elif alg_name == 'GCN':
            models = [
                STGCN(in_features=94)
            ]
        ...
        for network in (models):
            ...
            with torch.no_grad():
                if isinstance(model, (SetTransformer)):
                    ...
                    np.savetxt(log_dir + '/decoding_set_transformer' + '_trial_' + str(trial_i) + '_num_neib_' + str(num_neib) + '_r_' + str(r) + '.txt', decoding, delimiter=',')
                    np.savetxt(log_dir + '/output_set_transformer' + '_trial_' + str(trial_i) + '_num_neib_' + str(num_neib) + '_r_' + str(r) + '.txt', output, delimiter=',')

                if isinstance(model, (STGCN)):
                    output, decoding = model(features, adj_norm)
                    np.savetxt(log_dir + '/decoding_set_transformer_gcn' + '_trial_' + str(trial_i) + '_num_neib_' + str(num_neib) + '_r_' + str(r) + '.txt', decoding, delimiter=',')
                    np.savetxt(log_dir + '/output_set_transformer_gcn' + '_trial_' + str(trial_i) + '_num_neib_' + str(num_neib) + '_r_' + str(r) + '.txt', output, delimiter=',')
    ...
    if alg_name == 'vanilla' or alg_name == 'GCN':
        ...
        if alg_name == 'vanilla':
            df_test = pd.DataFrame(tests, columns=['SetTransformer'])
        elif alg_name == 'GCN':
            df_test = pd.DataFrame(tests, columns=['STGCN'])
    ...
    def get_performance_test_bayesian(trial_i):
        ...
        for network in (models):
            ...
            if isinstance(model, (STGCN)):
                embedding = np.loadtxt(log_dir + '/decoding_set_transformer_gcn' + '_trial_' + str(trial_i) + '_num_neib_' + str(num_neib) + '_r_0.txt', delimiter=",")
            ...
            np.savetxt(log_dir + '/output_set_transformer_bgcn' + '_trial_' + str(trial_i) + '_num_neib_' + str(num_neib) + '_r_' + str(r) + '.txt', output.detach().numpy(), delimiter=',')
    ...
    if alg_name == 'BGCN':
        df_test_bayesian.to_csv('accuracy_set_transformer_num_neib_' + str(num_neib) + '_r_' + str(r) + '_' + alg_name + '.csv', index=False)

    else:
        df_test.to_csv('accuracy_set_transformer_num_neib_' + str(num_neib) + '_r_' + str(r) + '_' + alg_name + '.csv', index=False)
