...
def run_rFF_pool_one_dataset(blank, data_index, num_neib, pooling, r, num_alg):
    ...
    def get_performance_test(trial_i, fold_j):
        ...
        if num_alg == 3:
            models = [
                rFF_pool(in_features=200, pooling_method=pooling),
                rFF_pool_GCN(in_features=200, pooling_method=pooling)
            ]
        elif num_alg == 2:
            models = [
                rFF_pool_GCN(in_features=200, pooling_method=pooling)
    ...
    if num_alg > 1:
        ...
        if num_alg == 3:
            df_test = pd.DataFrame(tests, columns=['rFF_pool', 'rFF_pool_GCN'])
        else:
    ...
    if num_alg == 1:
        df_test_bayesian.to_csv('accuracy_' + data_name + '_rFF_pool_num_neib_' + str(num_neib) + '_' + pooling + '_r_' + str(r) + '_only_BGCN.csv', index=False)
    else:
        df_concat = pd.concat([df_test, df_test_bayesian], axis=1)
        print(df_concat.mean(axis=0))
        if num_alg == 3:
            df_concat.to_csv('accuracy_' + data_name + '_rFF_pool_num_neib_' + str(num_neib) + '_' + pooling + '_r_' + str(r) + '.csv', index=False)
        else:
            df_concat.to_csv('accuracy_' + data_name + '_rFF_pool_num_neib_' + str(num_neib) + '_' + pooling + '_r_' + str(r) + '_GCN_BGCN.csv', index=False)
