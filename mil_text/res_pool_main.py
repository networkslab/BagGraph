...
def run_res_pool_one_dataset(blank, data_index, num_neib, pooling, r, alg_name):
    ...
    def get_performance_test(trial_i, fold_j):
        ...
        if alg_name == 'vanilla':
            models = [res_pool(in_features=200, pooling_method=pooling)]

        elif alg_name == 'GCN':
            models = [res_pool_GCN(in_features=200, pooling_method=pooling)]
    ...
    if alg_name == 'vanilla' or alg_name == 'GCN':
        ...
        if alg_name == 'vanilla':
            df_test = pd.DataFrame(tests, columns=['res_pool'])
        elif alg_name == 'GCN':
    ...
    if alg_name == 'BGCN':
        df_test_bayesian.to_csv('accuracy_' + data_name + '_res_pool_num_neib_' + str(num_neib) + '_' + pooling + '_r_' + str(r) + '_' + alg_name + '.csv', index=False)
    else:
        df_test.to_csv('accuracy_' + data_name + '_res_pool_num_neib_' + str(num_neib) + '_' + pooling + '_r_' + str(r) + '_' + alg_name + '.csv', index=False)
