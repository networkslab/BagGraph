...
def run_res_pool_one_dataset(blank, data_index, num_neib, pooling='max', r=5, num_alg=3):
    ...
    log_dir = 'log_' + pooling + '_res/' + data_name
    ...
    def get_performance_test(trial_i, fold_j):
        ...
        if num_alg == 3:
            models = [
                res_pool(in_features=dim_features, pooling_method=pooling),
                res_pool_GCN(in_features=dim_features, pooling_method=pooling)
            ]
        elif num_alg == 2:
            models = [
                res_pool_GCN(in_features=dim_features, pooling_method=pooling)
        ...
        for network in (models):
            ...
            if isinstance(model, (res_pool_GCN)):
                embedding = np.loadtxt(log_dir + '/decoding_res_pool_' + pooling + '_' +
            ...
            with torch.no_grad():
                if isinstance(model, res_pool):
                    ...
                    np.savetxt(log_dir + '/decoding_res_pool_' + pooling + '_'
    ...
    if num_alg > 1:
        ...
        if num_alg == 3:
            df_test = pd.DataFrame(tests, columns=['res_pool', 'res_pool_GCN'])
        else:
            df_test = pd.DataFrame(tests, columns=['res_pool_GCN'])
    ...
    df_test_bayesian = pd.DataFrame(tests_bayesian, columns=['res_pool_BGCN'])
    print(df_test_bayesian.mean(axis=0))

    if num_alg == 1:
        df_test_bayesian.to_csv('accuracy_' + data_name + '_res_pool_num_neib_' + str(num_neib) + '_' + pooling + '_r_' + str(r) + '_only_BGCN.csv', index=False)

    else:
        df_concat = pd.concat([df_test, df_test_bayesian], axis=1)
        print(df_concat.mean(axis=0))
        if num_alg == 3:
            df_concat.to_csv('accuracy_' + data_name + '_res_pool_num_neib_' + str(num_neib) + '_' + pooling + '_r_' + str(r) + '.csv', index=False)
        else:
            df_concat.to_csv('accuracy_' + data_name + '_res_pool_num_neib_' + str(num_neib) + '_' + pooling + '_r_' + str(r) + '_GCN_BGCN.csv', index=False)
